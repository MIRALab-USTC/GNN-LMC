...
class AsyncIOPool(torch.nn.Module):
    def __init__(self, pool_size: int, buffer_size_train: int, buffer_size_eval: int, embedding_dim: int):
        super().__init__()
    ...
    def _cpu_buffer(self, idx: int) -> Tensor:
        if self._cpu_buffers[idx] is None:
            self._cpu_buffers[idx] = torch.empty(self.buffer_size_train if self.training else self.buffer_size_eval,
                                                 self.embedding_dim,
                                                 pin_memory=True)
    ...
    def _cuda_buffer(self, idx: int) -> Tensor:
        if self._cuda_buffers[idx] is None:
            assert str(self._device)[:4] == 'cuda'
            self._cuda_buffers[idx] = torch.empty(self.buffer_size_train if self.training else self.buffer_size_eval,
                                                  self.embedding_dim,
                                                  device=self._device)
    ...
    def __repr__(self):
        return (f'{self.__class__.__name__}(pool_size={self.pool_size}, '
                f'buffer_size_train={self.buffer_size_train}, '
                f'buffer_size_eval={self.buffer_size_eval}, '
